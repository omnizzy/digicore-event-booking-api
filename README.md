...
A RESTful Event Booking API built with Spring Boot that allows users to create events, book seats, and manage bookings with proper business rules, validation, and pagination.

This is a backend-only system with full Swagger documentation.
...
- Cannot book if seats are full
- Same email cannot book same event twice
- Cancelling a booking frees a seat
- Event automatically closes when full
...
| GET | `/events/{eventId}/bookings` | Get bookings for an event |
...
Event status is either OPEN or CLOSED only
